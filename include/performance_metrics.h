...
class PerformanceMetrics{
    ...
    public:
        ...
        //MEMBER FUNCTIONS:
        // Setter.
        void add_image_detections(const std::vector<cv::Rect>& detection, const std::vector<cv::Rect>& labels, const std::vector<int>& emotions,
        const std::vector<int>& emotion_labels);

        // This member function compute the IOUs of the detected faces.
        std::vector<float> get_label_IOUs(const std::vector<cv::Rect>& detection, const std::vector<cv::Rect>& labels, std::vector<int>& ordering);
